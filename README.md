...
*   `Controllers` - The API entry point. Contains Minimal API endpoints and configuration.
...
## Running Tests
The core business logic (like the price calculator and booking overlap prevention) is covered by unit tests. To run them, use this command in the root folder:
```bash
dotnet test
```
